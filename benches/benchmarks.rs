...
pub fn blockchain_stack_serialize_100_blocks_variable_transaction_count(c: &mut Criterion) {
    ...
    let mut group =
        c.benchmark_group("blockchain_stack_serialize_100_blocks_variable_transaction_count");
    ...
    for num_transactions_per_block in [1, 5, 10, 25, 50, 100, 200, 500, 1000, 2500].iter() {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..*num_transactions_per_block {
                let transaction: Transaction =
                    Transaction::generate_miner_transaction(id.public_key);
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(
                (100 * num_transactions_per_block).to_string() + " Transactions",
            ),
            &blockchain,
            |b, blockchain| {
...
pub fn blockchain_stack_deserialize_100_blocks_variable_transaction_count(c: &mut Criterion) {
    ...
    let mut group =
        c.benchmark_group("blockchain_stack_deserialize_100_blocks_variable_transaction_count");
    group.plot_config(plot_config);

    let mut blockchain = Blockchain::genesis();

    for num_transactions_per_block in [
        1, 5, 10, 25, 50, 100, 200, 500, 1000, 2500, 5000, 10000, 25000,
    ]
    .iter()
    {
        // add 100 Blocks with num_transactions_per_block Transactions each to the blockchain
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(
                (100 * num_transactions_per_block).to_string()
                    + " Transactions from Bytes deserialized",
            ),
            &data,
            |b, data| {
...
criterion_group!(
    benches,
    blockchain_stack_serialize_100_blocks_variable_transaction_count,
    blockchain_stack_deserialize_100_blocks_variable_transaction_count
);
